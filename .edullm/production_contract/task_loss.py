...
# `eval/macro_bpb` (the flat mean over all 20 labels) weights MMLU at 40% --
# 8 of 20 labels are MMLU subject-group val/test splits, one of ten distinct
# benchmarks. Measured on paired control seeds (plan finding 1b): MMLU
# contributed 105% of the seed-to-seed gap in the flat macro (0.01183),
# while a per-benchmark macro (val/test averaged within each benchmark
# first, then meaned across the ten distinct benchmarks) gave a gap of only
# 0.00231 -- roughly 5x tighter. `eval/macro_bpb_per_benchmark` is therefore
# the PRIMARY registered endpoint (plan section 2's freeze contract);
# `eval/macro_bpb` is retained and reported only as a secondary/legacy
# comparison point. This grouping is fixed by benchmark name, not chosen
# post hoc.
_LABEL_SUFFIX = "_rc_5shot_bpb"


def _benchmark_of_label(label: str) -> str:
    if label.startswith("mmlu_"):
        return "mmlu"
    if not label.endswith(_LABEL_SUFFIX):
        raise AssertionError(f"label {label!r} does not end in {_LABEL_SUFFIX!r}")
    prefix = label[: -len(_LABEL_SUFFIX)]  # e.g. "arc_challenge_val" or "boolq_val"
    benchmark, split = prefix.rsplit("_", 1)
    if split not in ("val", "test"):
        raise AssertionError(f"label {label!r} has unexpected split segment {split!r}")
    return benchmark


_BENCHMARK_OF_LABEL: dict[str, str] = {
    label: _benchmark_of_label(label) for label in TASK_LOSS_RAW_LABELS
}
_DISTINCT_BENCHMARKS = tuple(sorted(set(_BENCHMARK_OF_LABEL.values())))
...
def task_loss_metrics(payload: Mapping[str, Any]) -> dict[str, float]:
    ...
    if task_loss_payload_complete(payload):
        # PRIMARY endpoint (plan section 2 freeze contract / finding 1b):
        # average each of the 10 distinct benchmarks first (val and test
        # together), then average across benchmarks, so MMLU counts once
        # rather than 8 times at 40% of the weight.
        per_benchmark: dict[str, list[float]] = {}
        for label in TASK_LOSS_RAW_LABELS:
            per_benchmark.setdefault(_BENCHMARK_OF_LABEL[label], []).append(labels[label])
        benchmark_means = {
            benchmark: sum(values) / len(values) for benchmark, values in per_benchmark.items()
        }
        metrics["eval/macro_bpb_per_benchmark"] = sum(benchmark_means.values()) / len(
            benchmark_means
        )
        for benchmark, mean in benchmark_means.items():
            metrics[f"eval/bpb_by_benchmark/{benchmark}"] = mean
        # Secondary/legacy: the flat mean over all 20 labels. Retained for
        # comparison only -- do not use this as the primary endpoint (it is
        # 40% MMLU and MMLU alone accounted for 105% of the measured
        # control-seed noise gap).
