#!/usr/bin/env bash
# Stage sealed inputs with a temporary aws-session.env, then delete credentials.
set -Eeuo pipefail
...
stage_args=(
  ...
  --headroom "${STAGE_HEADROOM:-1.25}"
)
if [[ -n "${STAGE_EXTRA_ARGS[*]:-}" ]]; then
  stage_args+=("${STAGE_EXTRA_ARGS[@]}")
fi
