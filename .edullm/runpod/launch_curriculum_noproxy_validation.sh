#!/usr/bin/env bash
# Launch dense OLMo2-370M quadratic-MTLD curriculum with no-proxy HPs on 8 GPUs.
set -Eeuo pipefail
...
RUN_SLOT="${RUN_SLOT:-quadratic-mtld-noproxy-hps-mb32k-v1}"
RECOVERY_MODE="${RECOVERY_MODE:-fresh}"
HARD_TIME_LIMIT="${HARD_TIME_LIMIT:-72h}"
# 250 GiB network volumes cannot satisfy the usual 300 GiB free check.
REQUIRED_FREE_GIB="${REQUIRED_FREE_GIB:-180}"

[[ "${RUN_SLOT}" =~ ^[A-Za-z0-9_.-]+$ ]] || { echo "invalid RUN_SLOT" >&2; exit 2; }
[[ -d "${REPO_DIR}/.git" ]] || { echo "missing checkout: ${REPO_DIR}" >&2; exit 2; }
[[ -f "${INPUT_MANIFEST}" ]] || { echo "missing staged curriculum manifest" >&2; exit 2; }
...
case "${RECOVERY_MODE}" in
  fresh)
    ...
    run_name="hpo-validation-olmo2-370m-quadratic-mtld-noproxy-hps-$(date -u +%Y%m%d-%H%M%S)"
...
[[ "${available_kib}" -ge "${required_kib}" ]] || {
  echo "insufficient free workspace: $((available_kib / 1024 / 1024)) GiB available, ${REQUIRED_FREE_GIB} GiB required" >&2
  exit 2
}
...
export WANDB_RUN_GROUP="hpo-validation-olmo2-370m-quadratic-mtld-noproxy-hps"
...
timeout --signal=TERM --kill-after=120s "${HARD_TIME_LIMIT}" \
  python3 "${REPO_DIR}/.edullm/runpod/curriculum_noproxy_validation_entrypoint.py" \
