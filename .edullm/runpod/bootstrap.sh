#!/usr/bin/env bash
# Prepare a credential-free RunPod image for the Skill-It OLMo-core branch.
set -Eeuo pipefail

readonly BRANCH="edullm/skillit-370m"
...
"${PYTHON}" -m pip install --quiet --no-cache-dir \
  -r "${REPO_DIR}/.edullm/requirements-skillit-eval.txt"
...
printf '%s\n' "${resolved}" > /workspace/edullm-bootstrap/skillit.commit
