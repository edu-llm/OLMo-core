#!/usr/bin/env bash
# Prepare a credential-free RunPod image for the MixLaw OLMo-core branch.
set -Eeuo pipefail

readonly BRANCH="edullm/mixlaw-validation-370m"
...
if [[ -f "${REPO_DIR}/.edullm/requirements-mixlaw-eval.txt" ]]; then
  sed -i '/^rich==/d' "${REPO_DIR}/.edullm/requirements-mixlaw-eval.txt"
fi
...
"${PYTHON}" -m pip install --quiet --no-cache-dir \
  -r "${REPO_DIR}/.edullm/requirements-mixlaw-eval.txt"
...
printf '%s\n' "${resolved}" > /workspace/edullm-bootstrap/mixlaw.commit
