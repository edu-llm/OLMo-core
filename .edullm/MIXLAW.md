...
This integration runs standard OLMo-core OLMo2-370M pre-training seven times.
The only experimental input that changes is the selected seven-domain weight
vector in `.edullm/mixlaw_recipe.json`. `mix01` is not an arm.
...
code is vendored here. The published `mix18` decimals sum to `0.9999`;
construction divides each vector by its own sum because OLMo-core requires
unit-sum target ratios. The checked-in source numbers remain unchanged.
...
from the maximum demand across all seven production recipes and the domain's
...
The public CLI requires one `--arm-index` from 0 through 6. It execs
...
(`olmo-mix-1124`), or the mix07 fixture for arm 1:
...
The platform currently permits one `p4d.24xlarge`, so submit the seven
