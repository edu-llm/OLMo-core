...
def stop_token_ids(base_model: str, tokenizer) -> list[int]:
    """Every id that ends an assistant turn, read from the model's own generation config.

    ``tokenizer.eos_token_id`` is a single id and is **not** the whole stop set. On
    ``OLMo-2-0425-1B-Instruct`` the two agree — generation_config says ``[100257]``, the
    tokenizer says ``100257`` — so this function is a no-op there and every published 1B
    number is unaffected.

    On ``Olmo-3-7B-Instruct`` they disagree, and passing the tokenizer's id is silently
    destructive. The chat template closes an assistant turn with ``<|im_end|>`` (100265), and
    generation_config lists ``[100265, 100257]``, but ``tokenizer.eos_token_id`` is still
    ``100257`` (``<|endoftext|>``) alone. Handing that single id to ``generate`` *overrides*
    the model's list, so the real turn terminator stops being a stop token: generation runs on
    to ``--gen_max_new`` and the model keeps writing, inventing a student reply and further
    tutor turns. ``skip_special_tokens=True`` then strips the ``<|im_end|>`` that would have
    made this obvious, so the judge receives one long multi-turn blob as if it were a single
    opening turn — which reads as "the adapter never learned to stop", uniformly, on every arm.
    """
    ids: list[int] = []
    try:
        from transformers import GenerationConfig
        raw = GenerationConfig.from_pretrained(base_model).eos_token_id
        ids = [raw] if isinstance(raw, int) else list(raw or [])
    except Exception as exc:                                          # noqa: BLE001
        print(f"  (no generation_config for {base_model}: {exc}; falling back to tokenizer)")
...
def main():
    ...
    eos_ids = stop_token_ids(args.base_model, tokenizer)
    print("stop ids : " + ", ".join(f"{i} ({tokenizer.convert_ids_to_tokens(i)})"
                                    for i in eos_ids), flush=True)
