...
acc  = np.array([r["acc"] for r in rows])       # RAW overall math accuracy (% of all items)
steps = [0 if p == "base" else int(p[1:]) for p in pt]

fig, ax = plt.subplots(figsize=(12, 4.3))       # flat + wide
p = ax.scatter(kl, acc, c=steps, cmap="viridis", s=170, zorder=3, edgecolor="k", linewidths=0.7)
...
ax.set_title("Forgetting is Predictable: Forward KL vs. Old Task (Math)", fontweight="bold")
...
fig.tight_layout()
fig.savefig(os.path.join(FIG, "fig_kl_mathacc.png"), dpi=150, bbox_inches="tight")
print("saved analysis/figures/fig_kl_mathacc.png | linear R2=%.3f" % r2)
