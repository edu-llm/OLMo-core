...
def build_config(opts) -> ExperimentConfig:
    ...
    warmup = (
        ...
        else round(opts.warmup_ratio * total_steps)
    )
    eval_interval_steps = max(
        1, round(opts.eval_interval_tokens / gbs)
    )  # archive: every 250M tokens
    wandb_project = (
        opts.wandb_project
    ...
    trainer_config = (
        TrainerConfig(
            ...
            load_path=opts.resume_from,
            load_strategy=LoadStrategy.always if opts.resume_from else LoadStrategy.if_available,
            load_trainer_state=True if opts.resume_from else None,
            load_optim_state=True if opts.resume_from else None,
            metrics_collect_interval=5,
            cancel_check_interval=5,
            max_duration=Duration.steps(total_steps),
            # Concurrent Gloo groups for async bookkeeping and async checkpoints deadlocked on
            # both 8xA100 arms. Keep rank coordination synchronous; this changes scheduling only,
            # not model state, data order, optimization, or the objective.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--mask-dir",
        default=os.environ.get("COLMLM_MASK_DIR"),
    ...
    p.add_argument(
        "--resume-from",
        default=os.environ.get("COLMLM_RESUME_FROM"),
        help="Checkpoint or checkpoint-root path from a previous run. Require and restore its "
        "model, optimizer, trainer, dataloader, RNG, scheduler, and global-step state while "
        "writing new checkpoints to --save-folder.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Train N epochs over the corpus (overrides --budget-tokens).",
    )
    p.add_argument(
        "--steps", type=int, default=None, help="Absolute step budget (overrides tokens/epochs)."
    )
    p.add_argument("--save-interval", type=int, default=3814, help="~250M tokens at 65,536/step.")
    p.add_argument(
        "--warmup-ratio", type=float, default=0.02, help="Warmup as a fraction of --steps."
    )
    p.add_argument(
        "--warmup-steps", type=int, default=None, help="Absolute warmup; overrides ratio."
    )
    p.add_argument("--learning-rate", type=float, default=3e-4)
    p.add_argument(
        "--global-batch-size", type=int, default=32 * 2048, help="Tokens/step (32 x 2048)."
    )
    p.add_argument(
        "--rank-microbatch-size", type=int, default=8 * 2048, help="Tokens/microbatch/rank."
    )
    ...
    p.add_argument(
        "--wandb-project",
        default=None,
        help="W&B project (else $WANDB_PROJECT/$EDULLM_WANDB_PROJECT).",
    )
    p.add_argument("--wandb-entity", default=None, help="W&B entity (else $WANDB_ENTITY).")
    p.add_argument(
        "--wandb-group",
        default=None,
        help="W&B group (else $WANDB_RUN_GROUP; e.g. memory-split-135m).",
    )
    p.add_argument(
        "--wandb-tags", nargs="*", default=[], help="Extra W&B tags (mode is added automatically)."
    )
    p.add_argument("--no-wandb", action="store_true", help="Disable W&B logging.")
    p.add_argument(
        "--compile", action="store_true", help="Enable torch.compile (needs a C compiler)."
    )
    p.add_argument(
        "--dry-run", action="store_true", help="Build and print the config, train nothing."
    )
    return p
...
def main() -> None:
    ...
    if opts.dry_run:
        ...
        rich.print(
            ...
            f"resume_from={config.trainer.load_path or 'none'} "
