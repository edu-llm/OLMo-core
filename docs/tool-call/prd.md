...
| Provenance | **28.75% reformat / 30% derived / 41.25% fresh** — i.e. **~59% of rows trace to a published, licence-verified record; ~41% we author** | Revised twice on 2026-08-08. Dolci is **dropped** (no licence tag). Replacements are all frontmatter-tagged and verified: `argilla/Synth-APIGen-v0.1` (apache-2.0, 49,402, **100% single-turn**), `MadeAgents/xlam-irrelevance-7.5k` (cc-by-4.0, 7,500), `nvidia/When2Call` (cc-by-4.0), `MU-NLPC/Calc-gsm8k` + `openai/gsm8k` (mit), `aialt/RetrievalQA` (mit), `eth-nlped/mathdial` (cc-by-4.0), `allenai/mathfish` (odc-by). **Reformat is capped near 29% structurally**, not by choice — web-search and pedagogy have no upstream containing a tool call at all |
| Generator model | **`Qwen/Qwen3-235B-A22B-Instruct-2507`** (Apache — verified at the **LICENSE file**, not just the tag), fallback **`Mistral-Small-3.2-24B`** (fits one 80 GB card) | And the number that reframes the question: **only 33% of rows need an LLM at all.** 67% is reformat, DSL interpretation, or programmatic generation. Arithmetic operands and every `expected_result` come from a Python stub, never a model. So the generator governs a third of the corpus and **none of its correctness** |
...
1. **~~Dolci licence sign-off~~ — WITHDRAWN, do not seek it.** `allenai/Dolci-Instruct-SFT-Tool-Use`
   has **no `license:` key at all** (verified: `cardData.license` absent), which is weaker than the
   prose reading. It is dropped, and a wider hunt found tagged, verified-single-turn replacements
   totalling 56,902 rows for general alone — so reformat *rose* to 28.75% while removing the one
   source that needed sign-off. See design §8.
2. **`nvidia/OpenMathInstruct-1`** — `license: other` (NVIDIA License). A grep found no NC clause but
   `other` is not a tag we accept on faith. v2 upside only; blocks nothing.
