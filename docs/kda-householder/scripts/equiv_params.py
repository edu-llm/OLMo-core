...
from olmo_core.nn.attention.recurrent import (
    KimiDeltaAttention, KimiDeltaAttentionConfig,
    KimiDeltaHouseholder, KimiDeltaHouseholderConfig,
)
OUT = {}

# ---------- F1: param count, config formula vs BUILT module, R = 1..4 ----------
print("=" * 104); print("F1: num_params() formula vs actually-built module, and the R multiplier")
...
print(f"  KimiDeltaAttention        formula={kda_formula:>10,d}  built={kda_actual:>10,d}  "
      f"match={kda_formula == kda_actual}", flush=True)
...
for R in (1, 2, 3, 4, 8):
    ...
    rows.append(dict(R=R, formula=f_, built=a_, match=f_ == a_,
                     ratio_vs_R1=None, ratio_vs_kda=f_ / kda_formula))
    print(f"  KimiDeltaHouseholder R={R} formula={f_:>10,d}  built={a_:>10,d}  match={f_ == a_}  "
          f"ratio_vs_KDA={f_ / kda_formula:.4f}", flush=True)
    del m
r1 = rows[0]["formula"]
for r in rows: r["ratio_vs_R1"] = r["formula"] / r1
print(f"\n  R=1 == KimiDeltaAttention param count: {r1 == kda_formula}  ({r1:,d} vs {kda_formula:,d})", flush=True)
...
print("\n" + "=" * 104); print("F1b: per-parameter shapes, R=1 vs R=4 (are the configs really different?)")
print("=" * 104, flush=True)
m1 = KimiDeltaHouseholderConfig(n_heads=n_heads, num_householder=1).build(d_model, layer_idx=0, n_layers=1)
m4 = KimiDeltaHouseholderConfig(n_heads=n_heads, num_householder=4).build(d_model, layer_idx=0, n_layers=1)
...
for n, (a, b) in sorted(diffs.items()): print(f"    {n:28s} R1={a}  R4={b}", flush=True)
...
OUT["kda_vs_hh1_names"] = dict(equal=set(kdan) == set(d1), only_kda=sorted(set(kdan) - set(d1)),
...
if torch.cuda.is_available():
    print("\n" + "=" * 104); print("F2: R=1 KimiDeltaHouseholder vs KimiDeltaAttention -- COMPUTATIONAL equivalence")
    print("      (same weights copied across; both fed identical input)")
    print("=" * 104, flush=True)
    torch.manual_seed(0)
    hh1 = KimiDeltaHouseholderConfig(n_heads=n_heads, num_householder=1, dtype=__import__(
        "olmo_core.config", fromlist=["DType"]).DType.bfloat16).build(d_model, layer_idx=0, n_layers=1).cuda()
    kda2 = KimiDeltaAttentionConfig(n_heads=n_heads, dtype=__import__(
        "olmo_core.config", fromlist=["DType"]).DType.bfloat16).build(d_model, layer_idx=0, n_layers=1).cuda()
    ...
    print(f"  max|diff| = {d.max().item():.3e}   rel = {d.max().item()/sc:.3e}   "
          f"bit-exact = {torch.equal(o_hh, o_kda)}", flush=True)
    OUT["r1_equiv"] = dict(copied=len(shared), total=len(sd_kda), unaligned=missing,
                           max_abs=d.max().item(), rel=d.max().item() / sc,
                           bit_exact=bool(torch.equal(o_hh, o_kda)))

# ---------- E: beta = 2*sigmoid, is beta==1 reachable? ----------
print("\n" + "=" * 104); print("E: beta = 2*sigmoid(w_b(x)) -- is beta == 1 an interior, reachable point?")
print("=" * 104, flush=True)
torch.manual_seed(1)
...
hhn.init_weights(init_method=__import__("olmo_core.nn.transformer.init", fromlist=["InitMethod"]
                                        ).InitMethod.normal, d_model=d_model, block_idx=0, num_blocks=1)
x = torch.randn(4, 256, d_model)
with torch.no_grad():
    pre = hhn.w_b(x)
    beta_neg = pre.sigmoid() * 2.0
    beta_pos = pre.sigmoid()
print(f"  allow_neg_eigval=True : beta range [{beta_neg.min():.6f}, {beta_neg.max():.6f}], "
      f"n within 1e-3 of 1.0 = {(beta_neg - 1.0).abs().lt(1e-3).sum().item()} / {beta_neg.numel()}", flush=True)
print(f"  allow_neg_eigval=False: beta range [{beta_pos.min():.6f}, {beta_pos.max():.6f}], "
      f"n within 1e-3 of 1.0 = {(beta_pos - 1.0).abs().lt(1e-3).sum().item()} / {beta_pos.numel()}", flush=True)
print(f"  code default for KimiDeltaHouseholderConfig.allow_neg_eigval = "
      f"{KimiDeltaHouseholderConfig().allow_neg_eigval}", flush=True)
...
OUT["beta"] = dict(neg_min=beta_neg.min().item(), neg_max=beta_neg.max().item(),
                   n_near_1_neg=int((beta_neg - 1.0).abs().lt(1e-3).sum()),
                   pos_min=beta_pos.min().item(), pos_max=beta_pos.max().item(),
...
print("\n" + "=" * 104); print("F3: LM-scale claim -- 52.1M non-embed (R=1) -> 71.2M (R=4), +37%")
print("=" * 104, flush=True)
print("  (mixer-only figures; the LM total also needs FFN + norms + lm_head, which the "
      "LM-side agent owns)", flush=True)
for d_m, nh, nl in [(512, 8, 12), (512, 8, 16), (512, 16, 12)]:
    ...
    print(f"  d_model={d_m} n_heads={nh} n_layers={nl}: mixer/layer R1={p1:,d} R4={p4:,d}  "
          f"x{p4/p1:.3f} ; all-layer delta = {(p4-p1)*nl/1e6:.2f}M", flush=True)
    OUT.setdefault("lm_scale", []).append(dict(d_model=d_m, n_heads=nh, n_layers=nl,
                                               p1=p1, p4=p4, ratio=p4/p1,
