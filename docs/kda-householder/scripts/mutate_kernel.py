...
MUTATIONS = [
    ...
    (
        "M01_rewalk_le",
        "R-walk off-by-one: `if j < i_r` -> `j <= i_r`",
        "                if j < i_r:",
        "                if j <= i_r:",
    ),
    (
        "M02_rewalk_reverse_order",
        "R-walk off-by-one: reverse index `R-1-i_rr` -> `R-1-i_rr-1` guarded",
        "            i_r = R - 1 - i_rr",
        "            i_r = max(R - 2 - i_rr, 0)",
    ),
    # --- dg placement (1) ---
    (
        "M03_dg_after_decay",
        "store dg AFTER the decay hand-off (b_dh already multiplied by b_a)",
        "        tl.store(p_dg, tl.sum(b_dh * b_s0, 1), mask=mask_k)\n        b_dh = b_dh * b_a[:, None]",
        "        b_dh = b_dh * b_a[:, None]\n        tl.store(p_dg, tl.sum(b_dh * b_s0, 1), mask=mask_k)",
    ),
    (
        "M04_dg_uses_pre_not_s0",
        "dg uses the PRE-decay state instead of S^(0)",
        "        tl.store(p_dg, tl.sum(b_dh * b_s0, 1), mask=mask_k)",
    ...
    (
        "M05_dk_drop_term2",
        "drop the second dk term (k inside w = k @ S^(r))",
    ...
    (
        "M06_last_eq_T",
        "`last = T - 1` -> `last = T` (pass-2 starts one token past the end)",
        "    last = T - 1",
        "    last = T",
    ),
    (
        "M07_last_eq_T_minus_2",
        "`last = T - 1` -> `last = T - 2`",
        "    last = T - 1",
        "    last = T - 2",
    ),
    ...
    (
        "M08_dec_pk_wrong",
        "pass-2 decrement: `p_k -= R*H*K` -> `p_k -= H*K` (missing R)",
        "        p_k -= R * H * K\n        p_v -= R * H * V",
        "        p_k -= H * K\n        p_v -= R * H * V",
    ),
    (
        "M09_dec_pv_wrong",
        "pass-2 decrement: `p_v -= R*H*V` -> `p_v -= H*V` (missing R)",
        "        p_v -= R * H * V\n        p_beta -= R * H",
        "        p_v -= H * V\n        p_beta -= R * H",
    ),
    (
        "M10_dec_pbeta_wrong",
        "pass-2 decrement: `p_beta -= R*H` -> `p_beta -= H` (missing R)",
        "        p_beta -= R * H\n        p_hs -= H * K * V",
        "        p_beta -= H\n        p_hs -= H * K * V",
    ),
    (
        "M11_dec_phs_wrong",
        "pass-2 decrement: `p_hs -= H*K*V` -> `p_hs -= K*V` (missing H)",
        "        p_hs -= H * K * V",
        "        p_hs -= K * V",
    ),
    (
        "M12_dec_pdk_wrong",
        "pass-2 decrement: `p_dk -= R*H*K` -> `p_dk -= H*K` (missing R)",
        "        p_dk -= R * H * K\n        p_dv -= R * H * V",
        "        p_dk -= H * K\n        p_dv -= R * H * V",
    ),
    (
        "M13_dec_pdv_wrong",
        "pass-2 decrement: `p_dv -= R*H*V` -> `p_dv -= H*V` (missing R)",
        "        p_dv -= R * H * V\n        p_db -= R * H",
        "        p_dv -= H * V\n        p_db -= R * H",
    ),
    # --- dropped masks (3) ---
    (
        "M14_drop_mask_dk_store",
        "drop mask on the dk store",
        "            tl.store(p_dk + i_r * H * K, b_dk, mask=mask_k)",
        "            tl.store(p_dk + i_r * H * K, b_dk)",
    ),
    (
        "M15_drop_mask_dq_store",
        "drop mask on the dq store",
        "        tl.store(p_dq, b_dq, mask=mask_k)",
        "        tl.store(p_dq, b_dq)",
    ),
    (
        "M16_drop_mask_hs_load",
        "drop mask on the pass-2 hs load",
        "        b_s0 = tl.load(p_hs, mask=mask_h, other=0.0).to(tl.float32) * b_a[:, None]",
        "        b_s0 = tl.load(p_hs).to(tl.float32) * b_a[:, None]",
    ),
    (
        "M17_drop_mask_dg_store",
        "drop mask on the dg store",
        "        tl.store(p_dg, tl.sum(b_dh * b_s0, 1), mask=mask_k)",
        "        tl.store(p_dg, tl.sum(b_dh * b_s0, 1))",
    ),
    # --- dbeta missing its i_v slot (1) ---
    (
        "M18_dbeta_no_iv_slot",
        "dbeta pointer drops its per-i_v slot (partials collide)",
    ...
    (
        "M20_drop_scale_dh",
        "drop `scale` from the dh accumulation",
    ...
    (
        "M25_dh_sign",
        "dh accumulation uses +b_du instead of b_dw (sign/scale error)",
        "            b_dh += b_k[:, None] * b_dw[None, :]",
        "            b_dh += b_k[:, None] * b_du[None, :]",
    ),
    (
        "M26_no_debug_barrier",
        "remove tl.debug_barrier() between the hs write and read",
        "    tl.debug_barrier()",
        "    pass  # barrier removed",
    ),
    (
        "M27_dq_uses_s0",
        "dq reads S^(0) instead of S^(R)",
        "        b_dq = scale * tl.sum(b_sr * b_do[None, :], 1)",
...
def mk(B, T, H, K, V, R, seed=0, alog=1.0, neg=False, cu=None):
    ...
    a = (
        torch.rand(H, generator=gen, device=DEV) * (alog - 1.0) + 1.0
        if alog > 1.0
        else torch.ones(H, device=DEV)
    )
...
def grads(fn, cfg, backend):
    ...
    q, k, v, g, beta, do, cus = mk(
        **kw,
        alog=cfg.get("alog", 1.0),
        neg=cfg.get("neg", False),
        cu=cfg.get("cu"),
        seed=hash(str(cfg)) % 10000,
    )
    leaves = [q, k, v, g, beta]
...
for name, cfg in REGIMES:
    ...
    print(
        f"  {name:16s} "
        + "  ".join(f"{n}={r:.2e}" for n, r in zip(NAMES, rels))
...
print("=" * 108)
print(
    f"MUTATION TESTING on the REAL TRITON KERNEL: {len(MUTATIONS)} mutations x {len(REGIMES)} regimes"
)
...
for mid, desc, old, new in MUTATIONS:
    if BASE.count(old) != 1:
        ...
        RESULTS.append(
            dict(id=mid, desc=desc, verdict="BAD_PATTERN", detail=f"count={BASE.count(old)}")
        )
        continue
    ...
    try:
        ...
        RESULTS.append(
            dict(id=mid, desc=desc, verdict="CAUGHT", detail=f"import {type(e).__name__}")
        )
        continue
    for name, cfg in REGIMES:
        if caught_by:
        ...
        try:
            ...
            evidence[name] = f"raised {type(e).__name__}"
            continue
        worst, worst_n = 0.0, ""
        bad = False
        for n, gg, rr in zip(NAMES, got, REF[name]):
            if not torch.isfinite(gg).all():
                bad = True
                worst, worst_n = float("inf"), n + "(nonfinite)"
                break
            sc = rr.abs().max().item()
    ...
    print(
        f"  {mid:24s} {verdict:26s} caught_by={len(caught_by)}/{len(REGIMES)}  {desc[:60]}",
        flush=True,
    )
    if not caught_by:
        for name in evidence:
            print(f"      {name:16s} {evidence[name]}", flush=True)
    RESULTS.append(dict(id=mid, desc=desc, verdict=verdict, caught_by=caught_by, evidence=evidence))
    del mod
    sys.modules.pop(mid.lower(), None)
...
print(
    ...
    f"({len(RESULTS) - len(n_real)} skipped as bad patterns)",
    flush=True,
)
for r in n_real:
    if r["verdict"] != "CAUGHT":
...
with open(os.environ["OUT_JSON"], "w") as f:
    json.dump(
        dict(baseline_ok=base_ok, results=RESULTS, n_caught=len(n_caught), n_real=len(n_real)),
        f,
        indent=1,
    )
