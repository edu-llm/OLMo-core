...
def run_probe(name, args=()):
    ...
    r = subprocess.run(
        [sys.executable, os.path.join(PROBES, name), *args],
        capture_output=True,
        text=True,
        timeout=3000,
...
print("=" * 100)
print("LEVEL 1: probes/naive_kda_householder.py vs fla's KDA at R=1")
...
print("=" * 100)
print("LEVEL 2: kda_householder_torch.py fp64 gradcheck (TORCH backend)")
print("=" * 100, flush=True)
...
def mk64(B, T, H, K, V, R, seed=0, rg=True):
    ...
    def rnd(*s):
        return torch.randn(*s, generator=gen, dtype=torch.float64)
...
for R in (1, 2, 3):
    ...
    def fn(*a, R=R):
        return kda_householder_torch(*a, num_householder=R)[0]
...
try:
    ...
    def fn(*a):
        return kda_householder_torch(*a, num_householder=R, cu_seqlens=cu)[0]
...
for R in (1, 2):
    ...
    print(
        f"  gradgradcheck (2nd order, torch backend) R={R}: {l2[f'gradgradcheck_R{R}']}", flush=True
    )
...
# ---------- LEVEL 3 ----------
print("=" * 100)
print("LEVEL 3: probes/manual_backward_check.py vs level-2 autograd")
print("=" * 100, flush=True)
...
# ---------- LEVEL 4 ----------
print("=" * 100)
print("LEVEL 4: probes/bwd_emulator.py vs level 3")
print("=" * 100, flush=True)
...
# ---------- LEVEL 6 ----------
print("=" * 100)
print("LEVEL 6: probes/gpu_bwd_accept.py vs level 2 on GPU")
print("=" * 100, flush=True)
...
# ---------- audit probes ----------
for nm in (
    "audit_exp2_mutation.py",
    "audit_exp6_rewalk.py",
    "audit_exp7_mem.py",
    "audit_exp14_determinism.py",
):
    print("=" * 100)
    print(f"AUDIT PROBE: {nm}")
    print("=" * 100, flush=True)
    rc, out = run_probe(nm)
