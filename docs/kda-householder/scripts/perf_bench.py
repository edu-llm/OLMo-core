...
def timeit(fn, warmup, iters, label):
    ...
    try:
        ...
        print(
            f"  {label:58s} {dt*1000:11.2f} ms  peak {mem:6.3f} GiB  (warmup={warmup} iters={iters})",
            flush=True,
        )
...
def bench_fla_gdp(B, T, H, K, V, R, warmup, iters):
    ...
    return timeit(
        ...
        warmup,
        iters,
        f"fla.chunk_gated_delta_product(PER-HEAD g) B{B} T{T} H{H} K{K} V{V} R{R}",
    )


p = torch.cuda.get_device_properties(0)
print(
    f"GPU {p.name} sm_{p.major}{p.minor} SMs={p.multi_processor_count} mem={p.total_memory/2**30:.1f} GiB"
)
...
for B, T, H, K, V in [(4, 2048, 4, 64, 64), (2, 8192, 4, 64, 64), (2, 2048, 8, 64, 64)]:
    ...
    rec(
        ...
        mem2,
        st2,
        "fla production CHUNKED KDA kernel, per-channel gate, R=1 only",
    )
...
for B, T, H, K, V, R in [
    ...
    print(
        f"      -> hs formula = {hs:.3f} GiB ; measured peak = {mem if mem else -1:.3f} GiB",
        flush=True,
    )
    rec(
        f"B{B} T{T} H{H} K{K} V{V} R{R}",
        "ours-triton",
        ms,
        mem,
        st,
