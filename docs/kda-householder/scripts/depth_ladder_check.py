...
print(f"{'layers':>6} {'n':>2} " + "".join(f"{('L'+l):>9}" for l in ["40","64","128","256","512"]) + "   h(tokens)")
for lay in (1,2,4,6):
    fs=sorted(glob.glob(f"/scratch/users/ericrcwu/kda/probes/results/depth{lay}-kda-s5_words-s*.json"))
    if not fs: continue
    ...
    for L in ["40","64","128","256","512"]:
        ...
        m=st.mean(vs); cells.append(m)
        if m<=0.999: hs.append(int(L)*(m-CH)/(1-CH))
    print(f"{lay:>6} {len(recs):>2} " + "".join(f"{100*c:8.2f}%" for c in cells) + f"   {st.mean(hs):7.1f}")
print()
