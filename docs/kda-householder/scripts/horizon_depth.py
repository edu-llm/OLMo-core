...
import glob,json,os,re,math,statistics as st
CH=1/120; LS=["40","64","128","256","512","1024","2048"]
...
for f in glob.glob("/scratch/users/ericrcwu/kda/probes/results/all_night/*.json"):
    ...
    if not m: continue
    j=json.load(open(f))
    if j["task"]!="s5_words": continue
    D[(j["num_householder"],int(m.group(1)),j["seed"])]=j
def hz(a,L): return L*(a-CH)/(1-CH)
def runh(R,lay,s):
...
print(f"{'layers':>6} {'n':>2} {'h(R1)':>7} {'h(R4)':>7} {'delta_h':>8} {'95% CI':>20} {'h(R4)/h(R1)':>11}")
rows=[]
for lay in (1,2,3,4):
    seeds=sorted({s for (r,l,s) in D if r==1 and l==lay} & {s for (r,l,s) in D if r==4 and l==lay})
    if not seeds: continue
    h1=[runh(1,lay,s) for s in seeds]; h4=[runh(4,lay,s) for s in seeds]
    d=[b-a for a,b in zip(h1,h4)]; n=len(d)
    m=st.mean(d); sd=st.stdev(d) if n>1 else 0.0
    ci=T.get(n-1,2.0)*sd/math.sqrt(n) if n>1 else 0.0
    print(f"{lay:>6} {n:>2} {st.mean(h1):7.1f} {st.mean(h4):7.1f} {m:+8.1f}  [{m-ci:+7.1f},{m+ci:+7.1f}] {st.mean(h4)/st.mean(h1):10.2f}x  {'SIG' if abs(m)>ci else 'ns'}")
