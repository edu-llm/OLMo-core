...
**4. When we need it by: now — as soon as capacity exists.**
Nothing else blocks this: the code is complete and pushed, the dataset is one CPU command, and the
only external dependency is AWS creds for the S3 base checkpoint. So **take whatever is available
first** rather than waiting for the ideal configuration:
- **Any single ≥40 GB bf16 GPU is enough to start.** Don't hold out for 5. Launch
  **A2 first, then A0** — Gate A is the A2−A0 depth curve, so those two alone produce the headline
  signal (~9 h combined at 5% MFU on an A100); A3/A4 (Gate B) and A1 (the floor) can follow on the
  same GPU or on capacity that frees up later.
- **Run the 10-minute calibration (runbook §0b) on the first box you get**, before committing to a
  long reservation — a 2%-vs-10% MFU error is a 5× swing in the hours above.
- **Preemptible/spot is a poor fit right now.** With no `--resume` flag a preemption restarts from
  the base checkpoint, so a spot instance can cost the entire ~9 h arm rather than one 500-step
  interval. If non-preemptible capacity isn't available soon, say so — adding `--resume` from the
  rolling checkpoints is a contained change to `train_arm` and worth doing first in that case.
- Cheaper-signal fallback if capacity is tight: `--steps 1500` on A2/A0 gives a shape-of-the-curve
  read in ~3 h instead of ~9. It is a weaker screen, not a substitute for the 5,000-step pilot.
