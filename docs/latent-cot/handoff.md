...
**4. When we need it by: _TBD — fill this in before sending._**
Not specified by the requester yet. The work is not blocked on anything else: code is complete and
pushed, the dataset is generated by one CPU command, and the only external dependency is AWS creds
for the S3 base checkpoint.
