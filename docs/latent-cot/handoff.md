...
Phase-8 driver (`train_driver.py`). On `latent-superposition-module` **no shared source file is
touched**: the hidden state the thought loop needs is captured by a forward hook on the last block
(`cot._capture_last_block`) rather than by a kwarg on `Transformer.forward`.
...
**No core-file edits.** The continuous-thought loop reads post-block hidden states through a forward
hook on the last block (`cot._capture_last_block`), so the diff against `main` touches only
`src/olmo_core/latentcot/`, `src/scripts/latentcot/`, `src/test/latentcot/`, `docs/latent-cot/`, and
the platform files `.edullm/{Dockerfile,run.yaml}` + `.gitignore`. Core never imports `latentcot`;
the dependency runs one way only. (Earlier branches carried a `return_hidden_states` kwarg on
`Transformer.forward` — see `module-prd.md` for why it was dropped.)
...
- Continuous thoughts are produced by forwarding with `input_embeddings=` (bypasses the lookup) and
  capturing the last block's output via a forward hook, then **passed through `cot.final_norm`**
  (the LM head's own norm) before being fed back — see gotchas.
...
- **The last block's output is the PRE-final-norm residual stream** (this model's final norm
