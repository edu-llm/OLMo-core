...
- Added `MuonHConfig` (registered `"muon_h"`) and `MuonWConfig` (registered `"muon_w"`) in `olmo_core.optim.hyperball`: Muon with a selectable post-orthogonalization constraint, either Hyperball's fixed-Frobenius-norm projection ([arXiv 2606.16899](https://arxiv.org/abs/2606.16899)) or standard decoupled weight decay. Both build one optimizer and share the same momentum and Newton-Schulz path, so the two are directly comparable. Includes `newton_schulz_msign`, batched over leading dimensions, and a `block_rows` param-group option that treats a stacked MoE expert tensor as one independent matrix per expert for `msign`, the radius, and `adjust_lr`. Unlike `MuonConfig`, this needs no external `dion` dependency.
- Added `TransformerConfig.olmo2_370M_moe`, a sparse counterpart to `olmo2_370M` matched on active parameters (373.9M active of 975.8M non-embedding, against the dense config's 371.3M) with everything but the feed-forward held equal.
