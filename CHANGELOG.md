...
- `SpeedMonitorCallback` now knows the peak BF16 FLOP/s of the L40S, L40, L4 and A10G, instead of falling through to the "assume A100" branch and reporting MFU against 312 TFLOP/s. The three Ada names are prefixes of one another and are matched longest-first, since `"L4" in "NVIDIA L40S"` is true.
- Corrected the L40S peak from 362.05 to 181.03 TFLOP/s, which halves every MFU this callback has reported on that card. There are two independent factors of two on these constants and only one of them is sparsity: the other is the accumulation format. On the consumer-class dies the tensor cores run FP32 accumulation at half the FP16-accumulate rate, and the datasheets quote the FP16-accumulate figure. NVIDIA's Ada whitepaper gives the AD102 rates directly — 330.3 TFLOP/s for FP16 with FP16 accumulate against 165.2 for FP16 or BF16 with FP32 accumulate — and its L40 appendix lists BF16 as `181 | 362` for the silicon whose datasheet headline is 362.05. Torch accumulates in FP32 unconditionally, so the old figure was a rate no training kernel could reach. The L4 (AD104) and A10G (GA102) constants are suspected high for the same reason and are left alone with a comment, since nothing here has run on either and the L40S's first wrong value came from reasoning by analogy.
