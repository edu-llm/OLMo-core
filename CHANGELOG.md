...
- `SpeedMonitorCallback` no longer reports MFU for an L40S, L40, L4 or A10G against the A100's peak. Those four cards had no entry and fell through a branch that assumed every unrecognized device was an A100, so an L40S run was measured against 312 TFLOP/s rather than 183 and read 1.7x low, and an A10G run against 312 rather than 70 and read 4.5x low. The peak lookup now lives in `get_device_peak_flops_per_second`, whose figures are the dense rate with FP32 accumulation, the only rate torch can reach. Two independent factors of two separate that rate from a datasheet headline — sparsity, and the half-rate FP32 accumulation of the consumer-class dies (Ampere GA10x, Ada AD10x) — and NVIDIA applies neither consistently: the L40S is published with FP16 accumulation and the L40, the same die, without. An unrecognized device now returns `None`, which logs a warning and omits MFU rather than reporting one computed against another card's peak; set `device_peak_flops_per_second` on the callback to restore it.
