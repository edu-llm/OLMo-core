...
- Added `src/scripts/downstream_lane/`: `check_export_is_servable.py` answers whether vLLM will load a model this repository builds, off a meta-device build and before any weights exist, by matching the architecture name `save_pretrained` would write against vLLM's own registry; `serve_exported_checkpoint.sh` brings vLLM's OpenAI-compatible server up on an exported directory and does not return until it answers.


### Fixed

- An exported MoE checkpoint can now be read back into OLMo-core. `_fuse_moe_expert_weights` stacks per-expert weights into the 3D parameter per projection that `transformers` wants on the way out, and nothing did the reverse, so `convert_state_from_hf` met a `mlp.experts.gate_up_proj` it had no template for and raised `Some state keys were not converted` for both projections of every layer. `_split_moe_expert_weights` unstacks them inside `load_hf_model`, keyed on the tensor's rank so a per-expert layout is left alone. The dropless permutation `convert_checkpoint_to_hf` applies to `w1` and `w3` before saving now has an inverse too, `undo_dropless_moe_reshape`, beside the export rather than inside the import because the permutation is a fact about the OLMo-core module and `load_hf_model` sees only a directory and a state dict. Without both halves every exported MoE was write-only, which is the path `open_instruct`'s `olmo_core_finetune` reloads weights through and therefore the reason no arm of a MoE experiment could be post-trained from its own artifact.
