...
- Added Hyper-Connections and Manifold-Constrained Hyper-Connections in `olmo_core.nn.hyper_connections`. A `HyperConnection` wraps one transformer sub-layer and gives it `n` parallel residual streams instead of one: a read-in gate mixes them down to a single branch input, the branch runs once, and a write-out gate broadcasts its output back while an `n x n` mixer `H_res` routes the streams among themselves. `ResidualMixerType` selects the mixer — `identity`, `unconstrained` (the original Hyper-Connections, kept as an instability control), `sinkhorn` (exact mHC, log-space Sinkhorn-Knopp), `birkhoff` (mHC-lite, a softmax over all `n!` permutation matrices) and `kronecker` (KromHC, `log2(n)` doubly stochastic 2x2 factors) — for 8 / 24 / 24 / 32 / 12 routing parameters per wrapped sub-layer at `n = 4`. Everything initialises to the unwrapped backbone, so a hyper-connected model produces bit-identical logits to the `reordered_norm` model it wraps until the symmetry-breaking noise is turned on, and all routing math runs in float32 whatever dtype the activations are in. `TransformerBlockType.hyper_connection` and `TransformerType.hyper_connection` build the block and the model; `Transformer.forward` gained an `expand_residual_streams`/`collapse_residual_streams` pair that is the identity on the single-stream path. Dynamic routing, MoE, tensor and pipeline parallelism are not implemented, and the block raises rather than applying a TP or CP plan written for a three-dimensional hidden state. `src/scripts/ablations/` holds a six-arm mixer ablation with a CPU dry run and a GATE_1 correctness check; see `docs/hc-ablation/README.md`, which is explicit that no training or evaluation numbers have been produced.
