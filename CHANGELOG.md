...
- `SpeedMonitorCallback` now knows the peak BF16 FLOP/s of the L40S, L40, L4 and A10G, instead of falling through to the "assume A100" branch and reporting MFU against 312 TFLOP/s. An L40S is 362.05 TFLOP/s dense, so every MFU figure measured on one was inflated by 1.16x; an A10G is 70, so its MFU was understated by 4.5x. The three Ada names are prefixes of one another and are matched longest-first, since `"L4" in "NVIDIA L40S"` is true. Following the v2.5.0 fix to the A100 constant, each entry is the dense peak: the L4, L40 and A10G literals are the sparsity-inclusive datasheet numbers times `dense_correction`, and the L40S is NVIDIA's own dense figure taken as it stands, because its sparse listing of 733 is rounded and is not twice it.
