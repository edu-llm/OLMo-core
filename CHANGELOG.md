...
- Added the Mamba-3 sequence mixer (`Mamba3MixerConfig`) and hybrid model (`Mamba3Config`, with `mamba3_hybrid_190M` and `mamba3_olmo3_370M` presets), which interleaves Mamba-3 SSM blocks with attention. `rotation_block_size` selects the transition-block size `b`: `2` reproduces the paper's abelian complex-diagonal transition (TC⁰), while `b >= 3` uses a non-solvable SO(3) rotation whose prefix product is a genuine NC¹ scan, giving the layer state-tracking power a diagonal SSM provably lacks. Dispatch prefers the fused `mamba-ssm` Triton kernel where eligible and falls back to a chunked or pure-PyTorch reference path otherwise; `rotation_scan_impl` chooses how the `b >= 3` prefix product is computed (a quaternion formulation is the fastest of these).
- Added `no_weight_decay_param_names`, which collects the fully-qualified names of parameters a module has tagged `_no_weight_decay` for use with `OptimGroupOverride`. Mamba-3 tags `A_log` and `dt_bias` under `exempt_timescale_params_from_weight_decay` (on by default, and recorded in the checkpoint config): decaying them moves the recurrence's timescale rather than regularizing a weight, pulling `|A|` toward 1 and `dt` toward `softplus(0)`, which squeezes the memory horizon from both ends.
- Added `theta_max` to `Mamba3MixerConfig`, bounding the per-step rotation angle via `theta_max * tanh(theta / theta_max)`. Unbounded angles let the rotation wind arbitrarily far per step and were observed to diverge mid-run at every `b`, not only `b >= 3`.
- Added `Mamba3Config.mamba3_faithful_olmo3_190M`, the same architecture on the OLMo-3-190M shell (`d_model` 768, 12 layers, 190,293,192 non-embedding parameters), as a cheaper second scale for the `b` ablation. Runtime goes as parameters times tokens, so at a fixed tokens-per-parameter ratio it is quadratic in the parameter count: 190M at Chinchilla is about a seventh of the work of 370M at 10B. Nothing about the mixer differs between the two scales, asserted field by field. Both now build through the shared `Mamba3Config.mamba3_faithful_hybrid`.
- Added `Mamba3Config.mamba3_faithful_olmo3_370M`, a 370M preset that builds *published* Mamba-3 SISO — expand 2, token-dependent decay, a `D` skip, a post-BCNorm head bias initialized to one, norm-before-gate output ordering, the `tanh(theta) * pi * dt` rotation over half the state, and a pre-norm block. `mamba3_olmo3_370M` departs from published SISO in seven ways beyond the rotation and is left untouched. Three deviations remain and are shared by every block size: `d_state=192` (128 is not divisible by three and cannot express `b=3` at all), a group-shared rotation timescale (per-head is a measured 18.8x on the `b=3` rotation), and a learned `A_log` baseline under the token-dependent decay. `Mamba3Config.mamba3_hybrid_like` grew optional pass-throughs for each of these mixer options; all default to `None`, meaning "leave the mixer at its own default", so no existing preset moved.
- Added `src/scripts/train/OLMo3/OLMo3-370M-mamba3-b-ablation.py`, the `b=2` versus `b=3` ablation at 370M on 10B Dolma tokens, and `.edullm/run-b-ablation.yaml` to launch it. Its `plan` and `verify` commands diff the two arms' configs leaf by leaf and refuse to run if they differ in anything but `rotation_block_size`, which is the claim the experiment rests on; the learning rate is pinned rather than derived, because the ladder formula reads the parameter count and the 0.16% angle-projection surcharge `b=3` necessarily carries would otherwise hand the two arms different rates. `MAMBA3_B2_VS_B3.md` audits both arms against the paper and the pinned `mamba_ssm` reference module.
...
- `Transformer.forward` now emits a one-time `UserWarning` when `labels` is bit-identical to `input_ids`, which means nobody shifted the labels and training has silently become a copy task. `olmo_core.data.utils.get_labels` performs the shift, and the `labels` docstring now says who is responsible for it.
- Building a train module now refuses a bfloat16 request on a device whose silicon has no bfloat16 arithmetic — pre-Ampere, i.e. compute capability below 8.0, of which the T4 is the one card with tensor cores. `TransformerTrainModuleConfig.build` calls the new `olmo_core.train.train_module.validate_precision_support`, which raises `OLMoConfigurationError` naming the card, the capability, the config fields that asked, and the two ways out. `torch.cuda.is_bf16_supported()` returns `True` on a T4 — when its capability test fails it falls back to allocating a bfloat16 tensor, which succeeds because the storage and conversions are implemented in software — so the new `olmo_core.utils.get_devices_without_bfloat16` reads the compute capability off every visible device instead, the same way the float8 path does. No CUDA, ROCm, and a device that cannot be queried are all left alone. Previously this lived in `.edullm/train_on_corpus.py` and so only protected the one entry point built from that file.
...
- Tests that read or write a real object store now skip when the store *refuses* the credentials present, not only when there are no credentials at all. Each of them already skipped on `NoCredentialsError`; a checkout whose CI holds credentials for a different account gets past that branch and is answered with a 401 or a 403 instead, which is the same "there is no bucket here we can use" and was failing the `Test` and `Test checkpoint` jobs. `olmo_core.testing.remote_store_access_or_skip` is the shared condition, and it re-raises anything that is not an access refusal. The bucket is now `OLMO_CORE_TEST_S3_BUCKET` / `OLMO_CORE_TEST_GCS_BUCKET`, defaulting to the previous `ai2-olmo-testing`, so naming a reachable bucket is all it takes to get the coverage back.
- `test_get_beaker_client_caching` now carries the same `BEAKER_TOKEN` skip as the rest of `beaker_test.py`, via the new `olmo_core.testing.requires_beaker`. It was the one test in that file that tried to build a client unconditionally.
- The HF roundtrip test now skips a model whose repository is gated to the token in use, instead of failing. A token that has not accepted a model's licence is refused rather than told there is no token, which is not a defect in the conversion code.
- `get_root_dir` no longer aborts when there is no Beaker token. It asks Beaker one question — does this `ai2/*` cluster mount weka — and falls back to `gs://ai2-llm`, which is what it already returns for every cluster that does not, rather than raising. This is what failed the whole `Test olmo3 ladder` job, whose `dry-run` launches nothing.
...
- Fixed four places in `Trainer` where a rank-local condition decided whether that rank entered a collective, any of which can hang a multi-rank run until the process group's deadline expires. `_log_metrics` returned early when *this* rank had recorded nothing and so skipped the `reduce_metrics` all-reduce the other ranks had entered; `training_complete` raised on a rank-local `_error`, taking one rank out of the training loop on its own; the cancel check was gated on a rank-local `_canceled`; and `run_bookkeeping_op` dropped an invocation when a previous one happened to still be running on that rank, a hazard `check_if_canceled` already warned about in a comment. Observed cost on an 8×A100 node: seven ranks reporting `Timed out waiting 1800000ms` in the final `reduce_metrics` half an hour after training finished all its steps, then a further quarter of an hour at the shutdown barrier, and in two runs the `max_steps` checkpoint never written although every earlier one was.
- `reduce_metrics` now settles which steps and metric names are being reduced between the ranks, instead of shaping its all-reduce from whatever the calling rank happens to hold. A rank with nothing recorded takes part and contributes an identity, a metric only some ranks know is reduced rather than mismatching tensor shapes, a mean divides by the number of ranks that actually reported it, and when no rank has anything every rank returns together. This costs one small `all_gather_object` per flush and removes the two that the inconsistent-metrics path used to make. Its `metrics_consistent` argument is now accepted and unused, since what is reduced is agreed rather than assumed.
- `run_bookkeeping_op` now raises `OLMoConfigurationError` for `allow_multiple=False` together with `distributed=True`. All three callbacks in the tree that ask to drop a repeat op (W&B, Comet, Beaker) already pass `distributed=False`, so nothing in-tree changes.
