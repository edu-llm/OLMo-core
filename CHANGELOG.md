...
- Added `MultiheadLatentAttention` (DeepSeek MLA) as a new `SequenceMixer`, registered as `"mla"` via `MLAConfig` in `olmo_core.nn.attention`. It jointly compresses keys and values into a low-rank latent of size `kv_lora_rank` (with an optional low-rank query compression via `q_lora_rank`) and carries positional information on a *decoupled* RoPE sub-vector of width `qk_rope_head_dim` that bypasses the compression and is concatenated with the reconstructed non-RoPE part of width `qk_nope_head_dim`; the value head width is `v_head_dim`. This shrinks the inference KV cache to `kv_lora_rank + qk_rope_head_dim` numbers per token (versus `2 * n_kv_heads * head_dim` for standard MHA/GQA) while still computing full multi-head attention. It reuses the existing RoPE, `RMSNorm`, and SDPA-backend components. Tensor/context parallelism and an optimized latent-cache inference path are not yet implemented.
