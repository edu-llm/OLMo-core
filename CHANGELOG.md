...
- Added `GatedDeltaNet2` and `GatedDeltaNet2Config` (registered as `"gated_delta_net_2"`), the sequence mixer from [Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention](https://arxiv.org/abs/2605.22791). Where `GatedDeltaNet` drives the erase and the write of each state edit from one scalar `beta_t` per head, GDN-2 splits them into two independent channel-wise gates — an erase gate over the key axis and a write gate over the value axis — on top of KDA's channel-wise decay, so the decay bias is per key channel rather than per head and comes from a low-rank bottleneck. Collapsing both gates to a shared scalar recovers KDA; collapsing the decay as well recovers `GatedDeltaNet`. Ulysses context parallelism is supported (the gates are exchanged as flat channel tensors and the per-head decay rate is folded in before the exchange); tensor parallelism is not, matching `GatedDeltaNet`. Two defaults deliberately differ from `GatedDeltaNetConfig` and follow the paper instead: `expand_v` is `1.0` rather than `2.0` (the reference implementation's ratio), and `allow_neg_eigval` is `False` rather than `True` (the headline model keeps the erase gate in `[0, 1]`; Table 5 finds the widened `[0, 2]` range gives no consistent gain at 1.3B). The output gate is swish/SiLU per §3.5 and NVIDIA's reference layer, not the sigmoid that `fla.layers.gdn2` substitutes.
- Bumped the `fla` extra from `flash-linear-attention==0.4.1` to `==0.5.2`, which is the first release carrying `fla.ops.gdn2`.
...
- `GatedDeltaNet.num_flops_per_token` now counts the backward pass. It used `2 * params` (forward only) where `Attention`, `FeedForward` and `LMHead` all use `6 * params` — "2 ops * 3 for forward+backward" — so a GDN block's mixer was undercounted 3x while the feed-forward beside it was not. The visible effect was a model-level FLOPs-per-token that mixed the two conventions, and therefore a reported MFU and throughput that were wrong by whatever fraction of the model the mixer was. **This changes the throughput and MFU numbers reported for existing GDN runs**, so a comparison in flight against pre-change numbers should either re-derive both sides or pin the old commit. Losses, checkpoints and the model itself are untouched: nothing here feeds the training computation. `GatedDeltaNet2` uses the corrected convention from the start.
