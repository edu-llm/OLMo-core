...
## EduLLM P4 blocked vs. interleaved

The P4 blocked-vs-interleaved experiment code is in [`p4/blocked-vs-interleaved/`](p4/blocked-vs-interleaved/). It trains matched blocked and interleaved arms on the same four synthetic skill categories and reports held-out exact-match accuracy. The completed FarmShare run found 27.44% accuracy for blocked training and 61.62% for interleaved training, a +34.18 percentage-point interleaving effect. The dataset was generated by the experiment script: 16,384 synthetic training examples and 1,024 held-out test examples, with zero train/test skeleton overlap.
