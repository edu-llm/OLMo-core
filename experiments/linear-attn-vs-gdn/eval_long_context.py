...
def _load_model(checkpoint: str, device: torch.device, cache_dir: str):
    ...
    os.makedirs(cache_dir, exist_ok=True)
    cfg_local = os.path.join(cache_dir, "config.json")
    import subprocess

    subprocess.run(["aws", "s3", "cp", f"{checkpoint.rstrip('/')}/config.json", cfg_local], check=True)
...
def main():
    p = argparse.ArgumentParser(description="Long-context (increasing seq-len) breaking-point eval.")
    ...
    p.add_argument("--micro-batch", type=int, default=1, help="Windows per forward (keep 1 for long L).")
    ...
    p.add_argument("--upload-to", type=str, default=None, help="Optional s3:// path to upload results JSON.")
    ...
    try:
        ...
        if opts.wandb:
            ...
            run = wandb.init(
                entity="eduLLM", project="pretraining",
                name=f"{opts.run_name}-longctx",
                config=dict(checkpoint=opts.checkpoint, seq_lens=seq_lens,
        ...
        for L in seq_lens:
            ...
            if len(windows) == 0:
                ...
                break
            mean_ce, n_tok, buckets = _eval_length(
                model, windows, device, opts.micro_batch
            )
            ppl = math.exp(mean_ce)
            if baseline_ppl is None:
                baseline_ppl = ppl
            x_base = ppl / baseline_ppl
            print(f"{L:>10} {len(windows):>8} {n_tok:>12,} {mean_ce:>9.4f} {ppl:>12.3f} {x_base:>8.2f}")
            pos_curve = {
                b * BASE_LEN: (s / c if c else None) for b, (s, c) in sorted(buckets.items())
            }
            results.append(dict(seq_len=L, n_windows=int(len(windows)), n_tokens=n_tok,
                                mean_ce=mean_ce, ppl=ppl, x_baseline=x_base,
                                pos_ce=pos_curve))
            if run is not None:
                run.log(dict(seq_len=L, ppl=ppl, mean_ce=mean_ce, x_baseline=x_base), step=L)

        summary = dict(run_name=opts.run_name, checkpoint=opts.checkpoint,
                       num_params=model_cfg.num_params,
                       num_non_embedding_params=model_cfg.num_non_embedding_params,
                       base_len=BASE_LEN, results=results)
        ...
        if opts.upload_to:
            ...
            subprocess.run(["aws", "s3", "cp", out_path, opts.upload_to], check=True)
