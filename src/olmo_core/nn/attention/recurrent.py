...
from olmo_core.nn.attention.kda_api import dispatch_chunk_kda
...
def _init_short_conv(
    ...
    **The convolution weight is drawn first, unconditionally, from the shared generator**, so a
    ...
    .. warning::
        ...
        ``for module in self.modules(): module.reset_parameters()`` **before** the seeded
        generator is created, so that sweep draws from the **global** RNG. ``"lowrank"`` adds
        three ``nn.Linear`` submodules per convolution -- nine per layer -- each with its own
        ``reset_parameters``, so the global RNG lands in a different state than the plain arm's.
        Parameter *values* are still safe (they all come from the passed generator), but anything
        downstream that draws from the global RNG is not.
...
def _init_kda_timescales(
    ...
    *,
    ...
    """
    Draw the reference KDA timescales: ``A_log = log(U(1, 16))`` with a zero ``dt_bias``.

    Both writes go through ``_apply_init`` because under FSDP these are sharded ``DTensor``
    parameters. A random op on a ``DTensor`` over a CPU mesh has no RNG tracker, so it ignores
    the generator it was handed and draws from each process's default one -- the run stops being
    reproducible in exactly the two parameters that set the recurrence's timescale, and nothing
    raises. ``_apply_init`` draws the *full* tensor once and copies out the local shard, so one
    rank and many ranks produce the same numbers.

    Uniform on ``(1, 16)`` rather than ``(0, 16)``: ``log(0)`` is ``-inf`` and ``exp(A_log)`` is
    the decay rate, so a zero sample would silently freeze one head's state for the whole run.

    :param a_log: The per-head log decay scale.
    :param dt_bias: The per-channel gate bias.
    :param generator: The random generator, shared across the whole model.
    """
    from olmo_core.nn.transformer.init import _apply_init

    def draw_a_log(t: torch.Tensor) -> None:
        nn.init.uniform_(t, a=1.0, b=16.0, generator=generator)
        t.log_()

    _apply_init(draw_a_log, a_log)
    _apply_init(lambda t: t.zero_(), dt_bias)
    ...
    *per-channel* one. Concretely, the decay applied to the recurrent state is
    ...
    def __init__(
        ...
        *,
        ...
        # These two set the recurrence's decay horizon, so decaying them moves a timescale rather
        # than regularizing a weight: 'A_log' -> 0 drives the per-head rate to 1 and 'dt_bias' -> 0
        # pins softplus at 0.693, squeezing the horizon from both ends. 'mamba_ssm' exempts the
        # same pair. The tag is inert until an 'OptimGroupOverride' names the parameters; collect
        # them with :func:`~olmo_core.nn.utils.no_weight_decay_param_names`.
    ...
    def init_weights(
        ...
        *,
        ...
        _init_kda_timescales(self.A_log, self.dt_bias, generator=generator)
...
class KimiDeltaHouseholder(SequenceMixer):
    ...
    def __init__(
        ...
        *,
        ...
        self.A_log = nn.Parameter(torch.empty(self.n_heads, dtype=dtype, device=init_device))
        self.dt_bias = nn.Parameter(torch.empty(self.key_dim, dtype=dtype, device=init_device))
        # Timescale parameters, exempt from weight decay for the reason given in
...
class GatedDeltaNet2(SequenceMixer):
    ...
    def __init__(
        ...
        *,
        ...
        self.dt_bias = nn.Parameter(torch.empty(self.key_dim, dtype=dtype, device=init_device))
        # Timescale parameters, exempt from weight decay for the reason given in
        # :class:`KimiDeltaAttention`. Without this the arm would decay its decay horizon under
        # AdamW while the SSM arms it is compared against do not -- an optimizer difference
        # masquerading as an operator difference.
        self.A_log._no_weight_decay = True  # type: ignore[attr-defined]
        self.dt_bias._no_weight_decay = True  # type: ignore[attr-defined]
