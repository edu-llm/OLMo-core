...
class TransformerConfig(ModelConfig):
    ...
    **Default ``False``. UNVALIDATED ON HARDWARE as of 2026-08-10** -- see
    ``src/test/nn/quantization_comm_test.py``. This is a transport optimization that is required
    to be *bitwise* identical to the uncompressed path, and it refuses rather than approximating
    when a shard boundary would split a TWN output row.
    """
    ...
    def maple_scaled(
        ...
        **kwargs,
        ...
        :param ternary_comm: All-gather **packed ternary** (2-bit trits + per-row ``alpha``)
            ...
            ``model.ternary_comm=true``, and accepted identically by every Maple factory
            (``maple_r0``..``maple_r3``, ``maple_m7b``, ``maple_m20``) because they all delegate
            here.

            **Defaults to ``False`` and must stay there until the hardware gate passes.**
            ``src/test/nn/quantization_comm_test.py::test_bitwise_identical_to_post_gather_two_ranks``
            is that gate: it requires the gathered parameters to be **bitwise** identical to
            today's post-gather path. Flipping the default is then a one-line change here.
            ...
            ``delta`` are computable locally and come out identical. Where that does not hold
            (a flattened expert weight whose shard boundary is not expert-aligned)
            :meth:`~olmo_core.nn.quantization_comm.TernaryCommSpec.assert_exact` **raises**. It
            is never approximated and never rescued with an all-reduce of row statistics: either
        ...
        # surface exists to prevent. Same reasoning as the `quantize`-without-L4 raise above.
