...
``window_size`` argument (``backend.py:634-648``) and never forms the masked-out blocks. Which makes
these tests the only thing standing between "the wheel installed" and "the saving is real", because
every way this can fail is silent:
...
Every test here needs a GPU and a real flash-attn, and skips otherwise. **A skipped test proves
nothing about the image** -- the build-time assertion in ``.edullm/Dockerfile`` is what covers the
container, and an A100 (sm_80) run is what covers the hardware. FarmShare is sm_89.
"""
