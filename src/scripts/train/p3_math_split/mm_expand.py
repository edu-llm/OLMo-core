...
class MM:
    ...
    def assertion_frame(self, mand) -> AssertionFrame:
        mandatory_variables = {data[1] for kind, _, data in mand if kind == "$f"}
        active_disjoint = self.active_d()
        mandatory_disjoint = {
            pair for pair in active_disjoint if pair <= mandatory_variables
        }
        return AssertionFrame(
            ...
            active_e=tuple(
                (hyp_label, tuple(expr)) for hyp_label, expr in self.active_e()
            ),
