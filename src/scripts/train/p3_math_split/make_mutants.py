...
def main():
    ...
    ap.add_argument(
        "--heldout", default=None, help="heldout.json; defaults to a sibling of --shard"
    )
        ...
        json.dump(
            {
                "facts": held,
                "seed": None,
                "policy": "STUB from make_mutants — synthetic probes, not a real " "held-out set",
            },
            open(hp, "w"),
            indent=1,
        )
    ...
    print("  every mutant MUST make src/test/scripts/p3_math_split/corpus_invariants_test.py fail")
