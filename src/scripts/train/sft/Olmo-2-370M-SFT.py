"""
SFT (supervised fine-tuning) for a 370M OLMo-2 model.

Adapted from ``Olmo-2-7B-SFT.py`` for the AlphaAI/edu-llm project:
  * model is ``olmo2_370M`` (must match the architecture of the pretraining checkpoint you load);
  * decoupled from Beaker so it runs via the plain ``train`` / ``dry_run`` path on our AWS SLURM
    cluster (the ``launch`` subcommand still requires Beaker and is unused here);
  * ``--gpus_per_node`` is configurable (our runs use 1/2/4 GPUs, not a fixed 8-GPU node);
  * default sequence length is 4096 to match the pretraining recipe.

Run without arguments to see usage. The base checkpoint you pass must be an ``olmo2_370M`` model
(e.g. the ``model_and_optim`` dir of a run produced by ``OLMo2/OLMo2-370M.py``).
"""
...
# Fallback GPU type for clusters not in CLUSTER_TO_GPU_TYPE (i.e. our AWS runs). Only used to
# decide whether to double the microbatch size on B200s, so an H100 default is safe.
...
class SFTConfig(Config):
    """
    Custom config class for the SFT run. Mirrors the 7B script's config so command-line overrides
    (``--...``) work the same way.
    """
    ...
    def build(
        ...
        *,
        ...
        # Off-Beaker clusters (our AWS runs) aren't in CLUSTER_TO_GPU_TYPE; default to H100.
        gpu_type = CLUSTER_TO_GPU_TYPE.get(cluster, DEFAULT_GPU_TYPE)
        ...
        # Must match the pretraining architecture (OLMo2/OLMo2-370M.py) so the checkpoint loads.
        model = TransformerConfig.olmo2_370M(vocab_size=tokenizer_config.padded_vocab_size())

        # Checkpoints go to S3 by default (override with --trainer.save_folder or the submit env).
        if save_folder is None:
            save_folder = f"{root_dir}/checkpoints/{user_name or 'local'}/olmo-sft/{run_name}"

        # Only build a Beaker launch config when Beaker is actually available (the `launch`
        # subcommand). Our AWS runs use `train`/`dry_run` and leave this as None.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the 370M model.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Validate the config on CPU (no GPUs needed):
  python %(prog)s dry_run test /path/to/base/step12345/model_and_optim aws \\
      --dataset_path s3://alphaai-edullm-data/sft/my-mix

  # Train via torchrun on the AWS SLURM cluster (submit.sh does this for you):
  python %(prog)s train sft-run01 s3://alphaai-edullm-checkpoints/base-run/step10000/model_and_optim aws \\
      --gpus_per_node=2 --dataset_path s3://alphaai-edullm-data/sft/my-mix \\
      --trainer.save_folder s3://alphaai-edullm-checkpoints/sft-run01
""",
    )
    ...
    parser.add_argument(
        "cluster", help="Cluster label. Use 'aws' for our SLURM runs (any non-'ai2/' string)."
    )
    parser.add_argument(
        "--seq_len",
        type=int,
    ...
    parser.add_argument(
        "--gpus_per_node",
        type=int,
        help="GPUs allocated to this run (world_size = num_nodes * gpus_per_node).",
    ...
    parser.add_argument("--budget", help="The beaker budget to use (only for the launch path).")
    parser.add_argument("--workspace", help="The workspace to run in (only for the launch path).")
    ...
    if args.cmd == "dry_run":
        ...
        if config.launch is None:
            raise OLMoConfigurationError(
                "The 'launch' subcommand requires Beaker (BEAKER_TOKEN). For our AWS SLURM "
                "cluster use the 'train' subcommand via src/scripts/aws/submit.sh instead."
            )
